...
export default {
  ...
  methods: {
    ...
    getLayoutThemeConfig() {
      const migrateKey = 'themeConfigPrevMigrateFooterDefaultOff';
      const localThemeConfig = Local.get('themeConfigPrev');
      if (localThemeConfig) {
        if (!Local.get(migrateKey)) {
          if (localThemeConfig.isFooter) {
            localThemeConfig.isFooter = false;
            Local.set('themeConfigPrev', localThemeConfig);
          }
          Local.set(migrateKey, true);
        }
        this.$store.dispatch('themeConfig/setThemeConfig', localThemeConfig);
        const styleText = Local.get('themeConfigStyle');
        if (styleText) document.documentElement.style.cssText = styleText;
      } else {
